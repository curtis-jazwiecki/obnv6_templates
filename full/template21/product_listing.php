        ...
        switch ($column_list[$col]) {
         ...
         $feed_status = is_xml_feed_product($listing[$x]['products_id']);
		  ...
		  $image = tep_small_image($listing[$x]['products_image'], $listing[$x]['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT,'class="subcatimages"');
            ...
            if (isset($HTTP_GET_VARS['manufacturers_id'])) {
			$lc_text='';
              $lc_image = '<table><tr><td style="border:1px solid #fff;" valign="bottom"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $HTTP_GET_VARS['manufacturers_id'] . '&products_id=' . $listing[$x]['products_id']) . '" >' .  $image . '</a></td></tr></table>';
            } else {
			$lc_text='';
...
*/
		...
		$text_display = '';
            ...
            if (isset($HTTP_GET_VARS['manufacturers_id']))
			  {
				$lc_text='';
            	$lc_image_med = '<table width="100%" style="align: center"><tr><td style="border:1px solid #fff;" valign="bottom"><a href="' . tep_href_link(FILENAME_PRODUCT_INFO, 'manufacturers_id=' . $HTTP_GET_VARS['manufacturers_id'] . '&products_id=' . $listing[$x]['products_id']) . '" >' .  str_replace("/small/","/medium/",$image) . '</a></td></tr></table>';
              }
			else
			  {
				$lc_text='';
